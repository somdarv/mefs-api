 ...
 * idempotency, the amount check, and — the one that matters most today — that an
...
final class PaystackTest extends TestCase
{
    ...
    public function test_confirming_a_basket_starts_a_payment_and_returns_the_checkout_url(): void
    {
        $this->withKeys();
        $this->fakeInitialize();
        ...
        $this->assertSame('https://checkout.paystack.com/xyz123', $response->json('data.payment.authorization_url'));
        ...
        // ⚠️ The amount comes from the ORDER, in pesewas, with no conversion anywhere.
        Http::assertSent(fn ($request) => $request['amount'] === 8000 && $request['currency'] === 'GHS');
    }
    ...
    public function test_a_gateway_outage_still_leaves_a_usable_order(): void
    {
        ...
        // keeping — deleting it makes "we tried three times" unknowable.
        $this->assertSame('pending', Payment::query()->firstOrFail()->status->value);
    }
    ...
    public function test_a_failed_charge_leaves_the_order_pending_not_failed(): void
    {
        ...
        // One attempt failing says nothing about the order — the customer can try again in
    ...
    /**
     * ⚠️ A BROWSER REDIRECT IS NOT PROOF OF PAYMENT. Anyone can visit the tracking URL, so
     * the answer comes from a server-to-server call with our secret key.
     */
    public function test_verify_asks_paystack_rather_than_trusting_the_redirect(): void
    {
        $this->withKeys();
        $this->fakeInitialize();
        ...
        Http::fake(['*/transaction/verify/*' => Http::response([
            'status' => true,
            'data' => ['status' => 'success', 'reference' => $payment->reference, 'amount' => 8000, 'channel' => 'card'],
        ], 200)]);
    ...
    public function test_verify_says_not_paid_when_paystack_says_not_paid(): void
    {
        $this->withKeys();
        $this->fakeInitialize();
    ...
    public function test_a_customer_can_restart_an_abandoned_payment(): void
    {
        $this->withKeys();
        $this->fakeInitialize();
        ...
        $this->postJson("/api/v1/orders/{$order->tracking_token}/payment")
            ->assertCreated()
            ->assertJsonPath('data.authorization_url', 'https://checkout.paystack.com/xyz123');

        $this->postJson("/api/v1/orders/{$order->tracking_token}/payment")->assertCreated();

        // One row per ATTEMPT. Overwriting the first would make a settlement line naming the
        // earlier reference unexplainable at month end.
    ...
    private function placeOrderOverHttp()
    {
        ...
        return $this->withHeader('X-Guest-Session', $session->json('data.guest_session'))
            ->postJson("/api/v1/checkout-sessions/{$session->json('data.token')}/confirm", [
                'order_type' => 'pickup',
